...
def sobre(request):
	html="<html><body>Proyecto de Ejmplo MDW</body></html>"
...
def usuarios(request):
	...
	recetas = Receta.objects.all()
	return render_to_response('usuarios.html',{'usuarios':usuario,'recetas':recetas}, context_instance=RequestContext(request))
...
def contacto(request):
	if request.method=='POST':
		...
		if formulario.is_valid():
			titulo = 'Mensaje desde el recetario de Maestro del Web'
			contenido = formulario.cleaned_data['mensaje'] + "\n"
			contenido += 'Comunicarse a: ' + formulario.cleaned_data['correo']
			correo = EmailMessage(titulo, contenido, to=['LuisAJimenezA@gmail.com'])
			correo.send()
